...
configuration CORS à faire côté Ollama. Défaut du dépôt : `qwen3:14b` (cible
déploiement **RTX 5080**). Sur un laptop sans assez de VRAM, ne tirez pas le
14B : créez `web/borne/.env.local` avec `VITE_OLLAMA_MODEL=qwen3:8b` (ou
`qwen3:4b` encore plus léger).
...
| `temperature` | croquis Wi-Fi | température cutanée, en °C | `temp_c` : moyenne des secondes fiables, de 25 à 43 °C |
| `steps` | croquis Wi-Fi | pas comptés depuis le démarrage du bracelet | `pas` : le plus haut compteur de la minute |

Un zéro n'est jamais moyenné, et une valeur hors des bornes physiologiques est écartée : elle coûte une seconde, pas le lot. La SpO₂, elle, se garde sur toute l'échelle, de 1 à 100 % : une saturation très basse est justement celle qu'il ne faut pas perdre. Les pas ne se moyennent pas : c'est un compteur, et zéro y est une valeur. Respiration et activité électrodermale restent `NULL`, parce qu'aucun bracelet ne les mesure ; température et pas aussi, quand le firmware ne les envoie pas. Une clé inconnue fait en revanche échouer le lot entier en `400`, volontairement : une clé que le firmware viendrait d'ajouter se déclare dans `trameSchema` (`server/src/validation.ts`), elle ne se perd pas en silence. Le serveur répond `202` avec le nombre de trames et de minutes reçues et, s'il en a écarté, les valeurs hors bornes, champ par champ, dans `ecartees` : elles ne comptent pas, mais l'émetteur sait qu'elles existent. Il refuse en `404` un résident ou un bracelet inconnu, et en `409` un bracelet qui n'est pas attribué à ce résident.

Les mesures arrivées se lisent au backoffice, onglet *Base* → `mesures`, ou au terminal :

```bash
npm run db:sql "SELECT mesure_at, fc_bpm, rmssd_ms, spo2_pct, temp_c, pas, qualite FROM mesures WHERE bracelet_id = (SELECT id FROM bracelets WHERE serie = 'BR-0448') ORDER BY mesure_at DESC LIMIT 10"
```
...
Le bracelet y envoie le même lot que la borne, ou une lecture seule, que le serveur horodate à l'arrivée — c'est ce que fait un croquis Arduino sans horloge. `resident`, `bracelet` et `bpm` sont obligatoires ; un `rmssd` absent ne donne pas de valeur, une qualité absente vaut `fair`. Le croquis de l'équipe envoie aussi la température cutanée, le compteur de pas et l'état de chute :

```json
{ "resident": "R-0448", "bracelet": "BR-0448", "bpm": 72.0, "spo2": 98.0, "temperature": 33.47, "steps": 1842, "fall": false }
```

Une température hors de 25 à 43 °C est écartée et renvoyée dans `ecartees`, sans faire refuser la lecture. `steps` compte les pas depuis le démarrage du bracelet : la minute, puis la journée, en gardent le plus haut. `fall` dit si le croquis détecte une chute, là où la trame BLE les compte. Son passage à `true` ouvre un signal critique, comme `POST /ingest/evenement`, et la réponse porte `"chute": "signal ouvert"` ; tant qu'il reste à `true`, les lectures suivantes répondent `"deja signalee"` sans en ouvrir d'autre.

Les lectures d'une même minute font une seule ligne de `mesures`, recalculée à chaque envoi. La carte « en direct » de la fiche, elle, montre chaque lecture telle qu'elle arrive : le serveur garde en mémoire les dix dernières minutes de lectures de chaque résident, que la carte trace. Ses sept cases — FC, SpO₂, variabilité, température, activité, pas et chute — restent toujours affichées. Sans valeur, une case écrit « donnée indisponible » si rien n'en arrive, bracelet muet ou champ absent de la trame ; « calibrage… » si la trame porte le champ sans valeur retenue — zéro, hors bornes, capteur en mise en route ; « contact faible » si le capteur le signale. Pour les distinguer, le serveur garde avec chaque lecture la liste des champs qu'elle portait (`envoyes`). La case chute passe au rouge dès qu'une lecture des dix minutes signale une chute, et y reste tant que le signal de la dernière chute est ouvert : clos par un médecin, elle écrit « signal clos » si le croquis dit encore `fall`, « aucune » dès qu'il ne le dit plus. Tant que le bracelet envoie, les tuiles SpO₂, variabilité, température et pas suivent sa dernière lecture, aux mêmes seuils ; la FC de repos, qui est le bas de la journée et non la FC du moment, garde sa valeur du jour. Les pas, eux, ne se jugent que sur une journée finie : tant que le bracelet remplit celle du jour, la tuile et le registre la disent « en cours », et c'est la veille qui décide de l'alerte. Le journal du serveur écrit une ligne par requête venue du réseau — code HTTP, nombre de trames, motif d'un refus, jamais une valeur mesurée : c'est ce qu'on lit pendant qu'on règle un bracelet, dont le moniteur série ne montre souvent que le code. Sous Windows, le pare-feu demande au premier lancement s'il laisse Node écouter : sans cette autorisation pour le réseau du bracelet, le port reste injoignable.
...
Le croquis Wi-Fi de l'équipe, qui n'est pas dans ce dépôt, envoie en plus la température cutanée, les pas et l'état de chute — voir [En Wi-Fi, sans borne](#en-wi-fi-sans-borne).
...
- **Le modèle de la borne exige Ollama sur la machine.** Sans lui, Sola répond par une phrase de repli — « l'équipe médicale est prévenue » si une alerte est partie, « appelle l'infirmerie B » si elle a échoué — et la barre d'état affiche « IA locale injoignable ». Défaut : `qwen3:14b`, prévu pour le PC de déploiement (RTX 5080, modèle entier en GPU). Sur le laptop de développement (RTX 4050, 6 Go), rester en `qwen3:8b` via `web/borne/.env.local` : le 8B n'y tient pas entièrement en VRAM (premier chargement ~10 s, puis 4 à 7 s par réponse) ; le 14B n'y est pas tiré. `qwen3.5:4b` tient entièrement sur 6 Go et répond en ~2 s, mais invente nettement plus : il n'est pas retenu.
...
- **La file de triage se lit du plus récent au plus ancien.** Gravité affichée, mais plus de critère de tri. L'horloge du jeu de démonstration est celle du poste au moment de `npm run db:demo` : régénérer la base après une longue pause si les heures de la file ont trop vieilli.
- **Sola ne déclenche aucune action, et la plupart de ses règles sont des consignes.** Le modèle ne peut ni prévenir la maintenance, ni régler la lumière, ni contacter quelqu'un. Seul le code de la borne, sur mots-clés, transmet une alerte à l'équipe médicale — urgence, violence, symptôme préoccupant, demande d'aide, symptôme léger (score ≥ 3) — et Sola ne le dit qu'une fois l'alerte enregistrée par le serveur de bord ; elle ne renvoie vers l'infirmerie que si l'alerte n'a pas pu partir. Une alerte ouverte force le résumé de fin en remontée, y compris en sévérité `info`. Trois garde-fous sont écrits dans le code (`web/borne/src/ia.ts`) : une urgence (douleur thoracique, gêne respiratoire, malaise, idée suicidaire, meurtre ou menace de violence) reçoit une réponse écrite à l'avance et force la sévérité `critique` du résumé ; une phrase qui prétend une action est remplacée ; une phrase qui nomme une maladie est retirée. Le reste — ne rien inventer, rester dans le sujet — n'est que consigne, et un modèle de 14 milliards de paramètres s'en écarte encore : conseil incongru, faute de français, supposition.
- **La détection d'urgence repose sur des mots-clés.** Elle reconnaît les formulations courantes (« douleur dans la poitrine », « du mal à respirer », « plus envie de vivre », « meurtre », « tuer quelqu'un »…) ; une formulation qu'elle ne connaît pas passe au modèle, qui n'a plus alors que sa consigne. Elle n'ouvre alors aucune alerte, et Sola, qui ne renvoie pas vers l'infirmerie, n'y envoie pas le résident : seul le résumé de fin d'échange peut encore la faire remonter. Un symptôme léger (score ≥ 3) ouvre un signal `info` et force quand même la remontée du résumé. Elle préfère le faux positif : une phrase de trop coûte moins qu'une urgence manquée.
- **Le bracelet ne remplit que cinq tuiles de la fiche.** Chaque trame reçue recalcule la ligne de son jour dans `mesures_jour`, que lisent la fiche et le registre, mais aucune ne porte la respiration ni l'activité électrodermale. Restent la FC de repos, la SpO₂, la variabilité, la température cutanée et les pas — ces deux derniers, seul le croquis Wi-Fi les envoie. Les autres constantes gardent la valeur du jeu de démonstration ; un jour que le bracelet est seul à écrire, leur tuile reprend la dernière valeur connue, datée, alerte comprise.
- **Le compteur de pas repart avec le bracelet, pas à minuit.** Le croquis compte depuis son démarrage, et la journée garde le plus haut compteur de ses minutes : un bracelet redémarré dans la journée n'en garde que le plus grand des deux comptes, et celui qui passe minuit sans redémarrer porte les pas de la veille au lendemain.
- **Les pas du jour ne se jugent que le lendemain.** Tant que le bracelet remplit la journée, la fiche et le registre la disent « en cours », et c'est la veille qui décide de l'alerte : un résident qui cesse de marcher aujourd'hui n'alerte que demain. Une journée reste en cours jusqu'à minuit UTC, même si le bracelet s'est tu le matin.
- **Une chute du croquis Wi-Fi se date à sa réception, et le serveur ne s'en souvient qu'en mémoire.** Le croquis ne dit pas quand elle a eu lieu : le signal porte l'heure de la lecture qui l'annonce. Seul le passage de `fall` à `true` ouvre un signal, et la dernière valeur de chaque bracelet ne vit qu'en mémoire : un serveur redémarré pendant une chute encore signalée en rouvre un, un doublon plutôt qu'une chute manquée.
...
- **L'historique d'une constante ne garde que la moyenne de chaque minute.** Seule la carte « en direct » montre chaque lecture, pendant dix minutes, depuis la mémoire du serveur. Au-delà, une valeur de quelques secondes se dilue dans sa minute, et le « plus bas » du jour ne la voit pas. Un redémarrage du serveur efface ces dix minutes : jusqu'à la lecture suivante, la carte reprend la moyenne de la dernière minute, et le dit.
...
- **Une partie de la trame est reçue sans être conservée.** `tst`, `waso`, `hrRest`, `fall`, `shake`, `beats` et `amp` sont validées puis écartées : aucune table ne les attend encore. En particulier, une chute comptée par le firmware BLE n'ouvre pas de signal. Seules en ouvrent un celle d'une lecture Wi-Fi et `POST /ingest/evenement`, que la borne n'appelle pas.
- **En « Échange », Sola lit un résumé de sola.db pour Lyam (R-0448), pas le dossier brut.** Au démarrage de la scène, la borne appelle `/borne/ia-contexte/R-0448` : particularités, constantes, sommeil, scores, alertes ouvertes. Le modèle s'en sert pour orienter ; il ne doit pas citer les chiffres. Le résumé clinique de fin d'échange reste produit à partir de la conversation (et de ce contexte) : un problème jamais dit à l'oral peut quand même y entrer s'il figure dans le dossier. Le bracelet en direct et le RAG NASA/ESA ne sont pas lus pendant l'échange. « Apaisement » et « Alerte » restent des scénarios scriptés.
...
| `POST /ingest/bracelet` | la borne, par son relais ; le bracelet lui-même, sur le port réseau | trames brutes du bracelet, une par seconde ; le serveur en fait une ligne par minute, et un signal critique quand une lecture Wi-Fi annonce une chute — [le contrat](#transmettre-au-serveur-de-bord) |
...
| `GET /api/residents/:code/direct` | la console | écran 03 : les lectures du bracelet sur dix minutes, sa dernière minute et le jour en cours, relus toutes les dix secondes |
