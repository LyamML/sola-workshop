...
- **La file de triage se lit du plus récent au plus ancien.** Gravité affichée, mais plus de critère de tri : avant 16:05, heure locale, les signaux de démonstration datés de l'après-midi (jusqu'à 16:03) passent devant une alerte que la borne vient d'ouvrir. Elle reste dans les compteurs et dans le registre.
- **Sola ne déclenche aucune action, et la plupart de ses règles sont des consignes.** Le modèle ne peut ni prévenir la maintenance, ni régler la lumière, ni contacter quelqu'un. Seul le code de la borne, sur mots-clés, transmet une alerte à l'équipe médicale — urgence, symptôme préoccupant, demande d'aide — et Sola ne le dit qu'une fois l'alerte enregistrée par le serveur de bord ; elle ne renvoie vers l'infirmerie que si l'alerte n'a pas pu partir. Trois garde-fous sont écrits dans le code (`web/borne/src/ia.ts`) : une urgence (douleur thoracique, gêne respiratoire, malaise, idée suicidaire) reçoit une réponse écrite à l'avance et force la sévérité `critique` du résumé ; une phrase qui prétend une action est remplacée ; une phrase qui nomme une maladie est retirée. Le reste — ne rien inventer, rester dans le sujet — n'est que consigne, et un modèle de 8 milliards de paramètres s'en écarte encore : conseil incongru, faute de français, supposition.
- **La détection d'urgence repose sur des mots-clés.** Elle reconnaît les formulations courantes (« douleur dans la poitrine », « du mal à respirer », « plus envie de vivre »…) ; une formulation qu'elle ne connaît pas passe au modèle, qui n'a plus alors que sa consigne. Elle n'ouvre alors aucune alerte, et Sola, qui ne renvoie pas vers l'infirmerie, n'y envoie pas le résident : seul le résumé de fin d'échange peut encore la faire remonter. Elle préfère le faux positif : une phrase de trop coûte moins qu'une urgence manquée.
